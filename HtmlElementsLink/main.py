...
class ModuleMain(SMCApi.Module):
    ...
    def start(self, configurationTool):
        ...
        self.href = SmcUtils.getString(configurationTool.getSetting("href"))
        ...
        if SmcUtils.isArrayContainObjectElements(objectArray):
            ...
            shape = next(iter(
                filter(
                    lambda s: s.type == "text" and s.name == self.id, objList)))  # type: Shape
            # if shape:
...
if(element){
    ...
    def genResponse(self):
        ...
        objOutput.fields.append(SMCApi.ObjectField(
            "htmlBody",
            "<a id=\"%s\" href=\"%s\" %s >%s</a>" %
            (self.id, self.href, self.elementAttrs, self.text)))
